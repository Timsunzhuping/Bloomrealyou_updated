...
The AI design panel works in fallback mode without provider keys. To enable real Doubao/Volcengine Ark generation, set these values only in the server-side `/opt/my-app/repo/.env.production` file:

```env
AI_PROVIDER=doubao
ARK_API_KEY=replace-on-server-only
ARK_BASE_URL=https://ark.cn-beijing.volces.com/api/v3
DOUBAO_TEXT_MODEL=your-ark-text-model-id
DOUBAO_IMAGE_MODEL=your-seedream-image-model-id
AI_GENERATION_DAILY_LIMIT=20
S3_PUBLIC_BASE_URL=http://101.47.76.12/uploads
```

Do not commit real keys. The browser calls BloomRealYou API endpoints only; it never calls Ark directly. Generated images are downloaded by the API and persisted to the configured S3/MinIO bucket, then exposed through Nginx at `/uploads/...`.

Useful checks after deployment:

```bash
curl -X POST http://101.47.76.12/api/ai/design-suggestions \
  -H "Content-Type: application/json" \
  -d '{"productId":"classic-cotton-tee","locale":"zh-CN","scene":"健身房开业 T 恤"}'

curl -I http://101.47.76.12/uploads/
```
